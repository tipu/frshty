...
def _build_line_items(config: dict, body: dict, work_entries: list[dict]) -> list[dict]:
    ...
    if b.get("include_daily_descriptions") and work_entries:
        ...
        for e in work_entries:
            d = date.fromisoformat(e["date"])
            lines.append(f"{d.strftime('%A')}, {d.strftime('%B')} {d.day} · {e.get('hours', 8)}h")
        description = "\n".join(lines) if lines else "Consulting services"
        items.append({"quantity": hours, "price": rate, "description": description})
