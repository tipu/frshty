...
class TestFixCiFailuresTask:
    ...
    def test_not_caused_by_us_no_increment(self, fake_config, tmp_state, tmp_log):
        ...
        with patch("core.tasks.tickets.make_platform", return_value=mock_platform), \
             patch("features.pr_ci.run_balanced", return_value='{"caused_by_us": false, "reason": "flaky"}'), \
...
class TestCheckInReviewFixFailedRetry:
    ...
    def test_fix_failed_comment_reprocessed_on_next_scan(
        ...
        with patch("features.tickets.make_platform", return_value=mock_platform), \
             ...
             patch("features.tickets.run_balanced", return_value=haiku_classify) as haiku, \
...
class TestCheckInReviewPushFailure:
    ...
    def test_repeated_push_failure_caps_instead_of_looping_forever(
        self, fresh_db, fake_config, tmp_state
    ):
        """A comment that asks for a squash makes the fix run rewrite history,
        so push_branch is rejected on every retry. Holding the cursor without a
        bound would rerun that fix every poll for ever. The push failure counts
        against the same MAX_PR_COMMENT_FIX_ATTEMPTS budget as a failed fix, so
        the retry stops and ticket_pr_comment_fix_capped says why."""
        ts = None
        with patch("features.tickets.log.emit") as emit:
            for _ in range(tickets.MAX_PR_COMMENT_FIX_ATTEMPTS):
                ts, mock_platform, comment = self._run(
        ...
        assert len(cap_events) == 1, (
            "expected exactly one ticket_pr_comment_fix_capped after "
            f"{tickets.MAX_PR_COMMENT_FIX_ATTEMPTS} rejected pushes, "
            f"got {len(cap_events)}"
        )
        assert cap_events[0].kwargs.get("meta", {}).get("comment_id") == comment["id"]

        cursor = ts.get("last_comment_ids", {}).get("repo/99", 0)
        assert cursor >= comment["id"], (
            "after the cap the cursor must advance so the ticket stops "
            f"reworking the same comment. got cursor={cursor}"
        )
