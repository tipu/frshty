...
class TestGateCommit:
    ...
    def test_strips_attribution_from_a_message_file(self, tmp_path):
        ...
        assert out["decision"] == "allow"
        assert SESSION_LINK not in (tmp_path / "msg.txt").read_text()
        assert len(_gate_events(item_id)) == 1

    def test_denies_a_message_that_is_nothing_but_attribution(self):
        item_id, sid = _mkrun("all attribution commit")
        out = work_launch.gate_commit(sid, 'git commit -m "Generated with Claude"', "/tmp")
        assert out["decision"] == "deny"
        assert "commit gate" in out["reason"]
        assert json.loads(_gate_events(item_id)[0]["payload"])["verdict"] == "fail"
...
class TestHookCommitGate:
    ...
    def test_hook_rewrites_an_attributed_commit(self, tmp_path):
        item_id, sid = _mkrun("hook commit strip")
        ...
        assert SESSION_LINK not in out["updatedInput"]["command"]
        assert len(_gate_events(item_id)) == 1

    def test_hook_denies_a_message_it_cannot_correct(self, tmp_path):
        item_id, sid = _mkrun("hook commit deny")
        r = self._run_hook({"session_id": sid, "hook_event_name": "PreToolUse",
                            "tool_name": "Bash", "cwd": str(tmp_path),
                            "tool_input": {"command": 'git commit -m "Generated with Claude"'}})
        assert r.returncode == 0, r.stderr
    ...
    def test_hook_gates_the_push_on_the_corrected_command(self, tmp_path):
        item_id, sid = _mkrun("hook commit then push")
