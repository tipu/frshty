...
class TestUnloadedInstances:
    ...
    def test_every_reported_ticket_has_an_absolute_link(self, monkeypatch):
        _mkticket("acme", "DEV-635")
        _mkticket("someclient", "DEV-635")
        _loaded(monkeypatch, "acme", base_url="http://acme.local")
        for contexts in ("", "someclient", "acme"):
            for link in work_tickets.links_for(
                    _row(1, "fix DEV-635", contexts), work_tickets.index()):
                assert link["url"] == "http://acme.local/tickets/DEV-635"
...
class TestLinksForATask:
    def test_a_named_ticket_that_exists_becomes_a_link(self):
        _mkticket("acme", "DEV-635", status="planning")
        item = _mkitem("finish DEV-635 and open the PR", contexts="acme")
        links = work_tickets.for_items(
            [_row(item, "finish DEV-635 and open the PR", "acme")])[item]
        assert [(link["key"], link["instance"], link["status"]) for link in links] == [
            ("DEV-635", "acme", "planning")]
    ...
    def test_a_key_two_instances_hold_and_no_project_reports_both(self):
        ...
        assert [link["instance"] for link in links] == ["acme", "someclient"]

    def test_a_today_task_links_through_scope_ref(self):
        _mkticket("acme", "DEV-700")
        row = _row(1, "Fix the failing build", "acme",
    ...
    def test_the_link_points_at_the_host_that_owns_the_ticket(self, monkeypatch):
        _mkticket("acme", "DEV-635")
        _loaded(monkeypatch, "acme", base_url="http://acme.local/")
        links = work_tickets.links_for(_row(1, "fix DEV-635", "acme"), work_tickets.index())
        assert links[0]["url"] == "http://acme.local/tickets/DEV-635"


class TestTasksForATicket:
    def test_lists_the_tasks_that_named_the_ticket(self):
        _mkticket("acme", "DEV-635")
        mine = _mkitem("finish DEV-635 and open the PR", contexts="acme")
        _mkitem("unrelated cleanup", contexts="acme")
        rows = work_tickets.tasks_for("acme", "DEV-635")
        assert [r["id"] for r in rows] == [mine]
        assert rows[0]["url"] == f"/tasks/{mine}"
        assert rows[0]["projects"] == ["acme"]

    def test_a_substring_match_is_not_a_task_of_this_ticket(self):
        _mkticket("acme", "DEV-63")
        _mkticket("acme", "DEV-635")
        long_one = _mkitem("finish DEV-635", contexts="acme")
        assert [r["id"] for r in work_tickets.tasks_for("acme", "DEV-63")] == []
    ...
    def test_another_instances_ticket_does_not_claim_the_task(self):
        ...
        assert work_tickets.tasks_for("acme", "DEV-635") == []
        assert len(work_tickets.tasks_for("someclient", "DEV-635")) == 1

    def test_newest_first(self):
        _mkticket("acme", "DEV-635")
        first = _mkitem("start DEV-635", contexts="acme")
        second = _mkitem("finish DEV-635", contexts="acme")
        assert [r["id"] for r in work_tickets.tasks_for("acme", "DEV-635")] == [second, first]

    def test_carries_the_state_and_the_report(self):
        _mkticket("acme", "DEV-635")
        item = _mkitem("finish DEV-635", contexts="acme")
        db.execute("UPDATE work_items SET state = 'needs_ack', summary = 'PR opened' WHERE id = ?",
                   (item,))
        row = work_tickets.tasks_for("acme", "DEV-635")[0]
        assert row["state"] == "needs_ack"
        assert row["note"] == "PR opened"

    def test_lists_every_task_the_board_would_pill(self):
        _mkticket("acme", "DEV-635")
        ids = [_mkitem(f"step {n} of DEV-635", contexts="acme") for n in range(101)]
        listed = {r["id"] for r in work_tickets.tasks_for("acme", "DEV-635")}
        pilled = {i for i, links in work_tickets.for_items(
            [_row(i, f"step {n} of DEV-635", "acme") for n, i in enumerate(ids)]).items()
            if links}
        assert listed == pilled == set(ids)

    def test_a_padded_scope_ref_agrees_with_the_board(self):
        _mkticket("acme", "DEV-700")
        item = _mkitem("Fix the failing build", contexts="acme")
        ...
        assert [r["id"] for r in work_tickets.tasks_for("acme", "DEV-700")] == [item]
    ...
    def test_an_empty_key_asks_nothing(self):
        assert work_tickets.tasks_for("acme", "") == []

    def test_an_unknown_key_asks_nothing(self):
        assert work_tickets.tasks_for("acme", "DEV-999") == []


class TestBoardApi:
    def test_the_board_row_carries_its_tickets(self):
        _mkticket("acme", "DEV-635", status="in_review")
        item = _mkitem("finish DEV-635", contexts="acme")
