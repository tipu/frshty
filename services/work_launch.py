...
TASK_ID_PROJECTS = ("frshty",)


def task_id_allowed(contexts) -> bool:
    """Whether a commit or pull request of this task may carry its work item id."""
    return all(k in TASK_ID_PROJECTS for k in project_keys(contexts))


def task_id_match(item_id: int, text: str) -> str:
    """The first place `text` names the work item id, "" when it does not."""
    found = re.search(rf"(?<![0-9]){item_id}(?![0-9])", text or "")
    if not found:
        return ""
    return text[max(0, found.start() - 20):found.end() + 20].strip()


def _task_id_rule(item_id: int, contexts) -> str:
    """The rule that keeps the work item id out of a project's git history.

    The id means something only on this board. A commit or a pull request in
    any other project's repository is read by people who never see the board,
    so there it must not carry the id. A task that selects several projects
    gets the rule when any one of them is not in TASK_ID_PROJECTS, because the
    board cannot tell which repository the commit lands in."""
    if task_id_allowed(contexts):
        return ""
    return (f"Never write the work item id {item_id} in a commit message, a pull "
            "request title or a pull request description, in any form: not as "
            f"a number, not as \"work item {item_id}\", not as \"work-{item_id}\". "
            "The branch name carries the id, and a merge commit message names "
            "the branch by default, so every `git merge`, `git pull` and pull "
            "request merge you run needs a message you write yourself, without "
            "the branch name. ")
...
def _start(item_id: int, plan: dict, slack: bool, brief: str) -> dict:
    ...
    with work_store.launch_lock:
        ...
        context = (
            ...
            + _task_id_rule(item_id, contexts)
...
_TASK_ID_PR_MERGE_DENY = (
    "Merge blocked by the work-layer merge gate: this pull request merge "
    "would write a merge commit that names the work item id {item_id}. The "
    "default merge commit subject names the branch, and the branch carries "
    "the id.\n\n"
    "Run the merge again with --subject and a subject about the change only, "
    "without the id or the branch name."
)

_PR_MERGE_TEXT_FLAGS = ("--subject", "-t", "--body", "-b")


def _pr_merge_names_id(item_id: int, command: str) -> bool:
    """Whether a pull request merge writes a merge commit that names the id:
    a --merge without its own --subject, or a subject or body that names it."""
    tokens = _tokenize(_strip_heredocs(command)) or []
    text: list[str] = []
    for i, tok in enumerate(tokens):
        if i and tokens[i - 1] in _PR_MERGE_TEXT_FLAGS:
            text.append(tok)
        elif tok.startswith(("--subject=", "--body=")):
            text.append(tok.split("=", 1)[1])
    if ("--merge" in tokens or "-m" in tokens) and not any(
            t in ("--subject", "-t") or t.startswith("--subject=") for t in tokens):
        return True
    return bool(task_id_match(item_id, "\n".join(text)))


def gate_merge(session_id: str, command: str, cwd: str = "") -> dict:
    ...
    the agent. A local `git merge` is checked for the work item id in its
    commit message. Returns {"decision": "allow"|"deny", "reason": str}."""
    local = _gate_local_merge(session_id, command, cwd)
    if local:
        return local
    ...
    if not task_id_allowed(item["contexts"]) and _pr_merge_names_id(item["id"], command):
        work_store.record_gate(session_id, "merge_gate", "fail",
                               {"command": command[:300], "task_id": item["id"]})
        return {"decision": "deny",
                "reason": _TASK_ID_PR_MERGE_DENY.format(item_id=item["id"])}
...
def _commit_segments(command: str, verb: str = "commit") -> list[list[str]]:
    """The tokens of every pipeline segment that runs `git <verb>`."""
    ...
    for tok in tokens + ["\n"]:
        if tok in _SHELL_SEPARATORS:
            if _segment_git(segment, verb) is not None:
                out.append(segment)
            segment = []
        else:
            segment.append(tok)
...
def _heredoc_bodies(command: str) -> str:
    """The body of every heredoc in `command`, the lines _strip_heredocs drops."""
    lines = command.split("\n")
    bodies: list[str] = []
    i = 0
    while i < len(lines):
        markers = [m.group(2) for m in _HEREDOC_RE.finditer(lines[i])]
        i += 1
        for marker in markers:
            while i < len(lines) and lines[i].strip() != marker:
...
def _message_args(tokens: list[str]) -> list[str]:
    """The -m values of one git segment, bundled short flags included."""
    found: list[str] = []
    for i, tok in enumerate(tokens):
        if tok.startswith("--message="):
            found.append(tok[len("--message="):])
        elif tok.startswith("-") and not tok.startswith("--") and "m" in tok[1:-1]:
            found.append(tok[tok.index("m", 1) + 1:])
        elif (i and _takes_a_message(tokens[i - 1])
              and not tokens[i - 1].endswith(("F", "--file"))):
            found.append(tok)
    return found


def _commit_message_text(command: str, cwd: str) -> str:
    """Every piece of text the `git commit` in this command line can take as
    its message: -m arguments, heredoc bodies and -F files.

    The rest of the command line is left out, because a worktree path or a
    branch name in a `cd` or a `git push` beside the commit is not message. A
    commit the segment walk cannot see, such as one inside `bash -c`, gets the
    whole command line read instead."""
    segments = _commit_segments(command)
    if not segments:
        return command
    parts: list[str] = []
    for tokens in segments:
        parts += _message_args(tokens)
    parts.append(_heredoc_bodies(command))
    for message_file in _commit_message_files(command, cwd):
        try:
            if message_file.is_file() and message_file.stat().st_size <= _MESSAGE_FILE_MAX:
                parts.append(message_file.read_text(errors="replace"))
        except OSError:
            continue
    return "\n".join(parts)


_TASK_ID_COMMIT_DENY = (
    "Commit blocked by the work-layer commit gate: the message names the work "
    "item id {item_id}.\n\n"
    "Matched: {match}\n\n"
    "The id means something only on the work board, and this project's "
    "history is read by people who never see the board. Write the message "
    "about the change only, without the id in any form, and commit again."
)


_MERGE_NO_COMMIT_FLAGS = frozenset((
    "--abort", "--quit", "--ff-only", "--squash", "--no-commit"))

_TASK_ID_MERGE_DENY = (
    "Merge blocked by the work-layer merge gate: this `git merge` would "
    "commit a message that names the work item id {item_id}. Its default "
    "message names the branch, and the branch carries the id.\n\n"
    "Run the merge again with -m and a message about the change only, "
    "without the id or the branch name. After a conflict, finish with "
    "`git commit -m` rather than `git merge --continue`."
)


def _gate_local_merge(session_id: str, command: str, cwd: str) -> dict | None:
    """Deny a local `git merge` whose commit message would carry the work item
    id on a project that keeps the id out. None when nothing is denied."""
    segments = _commit_segments(command, "merge")
    if not segments:
        return None
    item = work_worktree.session_item(session_id)
    if item is None or task_id_allowed(item["contexts"]):
        return None
    for tokens in segments:
        if any(t in _MERGE_NO_COMMIT_FLAGS for t in tokens):
            continue
        given = _message_args(tokens) + _message_files(tokens)
        text = "\n".join(_message_args(tokens)) + "\n" + _heredoc_bodies(command)
        for path in _message_files(tokens):
            try:
                text += "\n" + Path(os.path.join(cwd or ".", path)).read_text(errors="replace")
            except OSError:
                continue
        if given and not task_id_match(item["id"], text):
            continue
        work_store.record_gate(session_id, "merge_gate", "fail",
                               {"command": command[:300], "task_id": item["id"]})
        return {"decision": "deny",
                "reason": _TASK_ID_MERGE_DENY.format(item_id=item["id"])}
    return None
...
def gate_commit(session_id: str, command: str, cwd: str = "") -> dict:
    ...
    item = work_worktree.session_item(session_id)
    if item is not None and not task_id_allowed(item["contexts"]):
        leaked = task_id_match(item["id"], _commit_message_text(command, cwd))
        if leaked:
            work_store.record_gate(session_id, "commit_gate", "fail",
                                   {"command": command[:300], "task_id": leaked})
            return {"decision": "deny",
                    "reason": _TASK_ID_COMMIT_DENY.format(item_id=item["id"],
                                                          match=leaked)}
